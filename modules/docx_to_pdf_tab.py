"""
DOCX to PDF Tab - Convert Word documents to PDF files.
...
from .pdf_operations import convert_docx_to_pdf


class DocxToPdfTab(ttk.Frame):
    """Tab for converting DOCX files to PDF format."""
    ...
    def _setup_ui(self):
        ...
        # Drop zone for multiple DOCX files
        self.drop_zone = DropZone(
            ...
            single_file=False,
            drop_text="Drop DOCX files here",
        )
    ...
    def _create_header(self, parent):
        ...
        ttk.Label(header_frame, text="DOCX to PDF", style="Title.TLabel").pack(
            side=tk.LEFT
        )
        ttk.Label(
            header_frame,
    ...
    def _handle_files_dropped(self, files: list):
        ...
        for f in files:
            path = Path(f)
            if path.suffix.lower() == ".docx" and path not in self.files_to_process:
        ...
        if added > 0:
            ...
            self.log_area.log("Please drop Word (.docx) files", "warning")
    ...
    def _convert_files(self):
        ...
        if not self.files_to_process:
            messagebox.showwarning("No Files", "Please add DOCX files to convert.")
            return
        ...
        if len(self.files_to_process) == 1:
            output_path = filedialog.asksaveasfilename(
                ...
                initialfile=self.files_to_process[0].stem + ".pdf",
            ...
            if not output_dir:
                return
            output_paths = [
                Path(output_dir) / (f.stem + ".pdf") for f in self.files_to_process
            ]
        ...
        def convert():
            ...
            def finish():
                ...
                self.convert_btn.configure(state=tk.NORMAL, text="Convert to PDF")
