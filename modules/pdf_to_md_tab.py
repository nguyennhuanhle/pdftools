"""
PDF to Markdown Tab - Convert PDF files to Markdown format.
...
from .pdf_operations import convert_pdf_to_markdown


class PDFToMdTab(ttk.Frame):
    """Tab for converting PDF files to Markdown format."""
    ...
    def _create_header(self, parent):
        ...
        ttk.Label(header_frame, text="PDF to MD", style="Title.TLabel").pack(
            side=tk.LEFT
        )
        ttk.Label(
            header_frame,
    ...
    def _convert_files(self):
        ...
        if len(self.files_to_process) == 1:
            output_path = filedialog.asksaveasfilename(
                ...
                defaultextension=".md",
                initialfile=self.files_to_process[0].stem + ".md",
            ...
            if not output_dir:
                return
            output_paths = [
                Path(output_dir) / (f.stem + ".md") for f in self.files_to_process
            ]
        ...
        def convert():
            ...
            for i, (input_path, output_path) in enumerate(
                ...
                success, message = convert_pdf_to_markdown(input_path, output_path)
            ...
            def finish():
                ...
                self.convert_btn.configure(state=tk.NORMAL, text="Convert to MD")
